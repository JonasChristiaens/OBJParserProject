...
void Game::Initialize()			
{
	...
	GAME_ENGINE->SetTitle(_T("OBJParser"));	
...
void Game::Start()
{
	...
	m_TxtFileNameUPtr = std::make_unique<TextBox>();
	m_TxtFileNameUPtr->SetBounds(25, 25, 25 + TB_WIDTH, 50);
	m_TxtFileNameUPtr->Show();
...
void Game::CallAction(Caller* callerPtr)
{
	...
	if (callerPtr == m_BtnLoadUPtr.get())
	{
		tstringstream fullFileName;
		fullFileName << _T("Resources/");
		fullFileName << m_TxtFileNameUPtr->GetText();

		if (!std::filesystem::exists(fullFileName.str()))
		{
			//set textbox red.
			m_TxtFileNameUPtr->SetBackcolor(RGB(255, 0, 0));

			//remove all text
			m_TxtFileNameUPtr->SetText(_T(""));
		}
		else
		{
